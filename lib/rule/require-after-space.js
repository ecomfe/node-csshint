...
function checkProperty(event) {
    ...
    // 排除下列情况
    // src: url(fonts/icomoon.eot?#iefixwexquh) format('embedded-opentype'),
    //      url(fonts/icomoon.woff?wexquh) format('woff'),
    //      url(fonts/icomoon.ttf?wexquh) format('truetype'),
    //      url(fonts/icomoon.svg?wexquh#icomoon) format('svg');
    if (event.property.toString() !== 'src') {
    ...
    for (var i = 0; i < len; i++) {
        ...
        if (part.type !== 'uri') {
            if (lineCacheComma !== line) {
                ...
                // 排除 background-image: 2px 2px url(data:image/gif;base64,R0lGODlhCAAHAIABAGZmZv...); 这样的情况
                lineContent = lineContent.replace(/url\(.*\)/, '');
