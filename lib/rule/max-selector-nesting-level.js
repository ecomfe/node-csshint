...
function startRule(event) {
    ...
    for (var i = 0; i < len; i++) {
        ...
        for (var j = 0; j < partsLen; j++) {
            ...
            if (part.type === 'descendant') {
                descendantCount++; // 从 0 开始，先 ++

                // descendantCount 先 ++ ，所以这里是 ruleVal - 1
                if (descendantCount > ruleVal - 1) {
                    // invalidObj = selector;
                    invalidSelectorList.push(selector);
                }
            }
            else if (part.type === 'child') {
                childCount++; // 从 0 开始，先 ++

                // childCount 先 ++ ，所以这里是 ruleVal - 1
    ...
    if (invalidSelectorList.length) {
        invalidSelectorList.forEach(
            function (invalidSelector) {
                var line = invalidSelector.line;
                var col = invalidSelector.col;
                var lineContent = util.getLineContent(invalidSelector.line, fileContent);
