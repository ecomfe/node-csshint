            ...
            css.eachDecl(function (decl) {
                ...
                if (/^([a-z0-9]{3,6})$/.test(simpleColorStr)) {
                    ...
                    if (global.CSSHINT_HEXCOLOR_CASE_FLAG === 1) {
                        ...
                        var upperCaseMsg =  msg + ', Current project case is ' + chalk.magenta('UpperCase.');

                        result.warn(upperCaseMsg, {
                            node: decl,
                            ruleName: RULENAME,
                            line: line,
                            col: col,
                ...
                // 当前这个颜色值里面的字母全是大写 #FAFAFA
                    ...
                    if (global.CSSHINT_HEXCOLOR_CASE_FLAG === 0) {
                        ...
                        lineContent = util.getLineContent(line, source.input.css);
                        col = source.start.column + decl.prop.length + decl.between.length;
                        var lowerCaseMsg =  msg + ', Current project case is ' + chalk.magenta('LowerCase.');

                        result.warn(lowerCaseMsg, {
                            node: decl,
                            ruleName: RULENAME,
                            line: line,
                            col: col,
                ...
                // 当前这个颜色值里面的字母有大写也有小写 #faFafA
                    ...
                    var str = msg;
                    if (global.CSSHINT_HEXCOLOR_CASE_FLAG === 0) {
                        str += ', Current project case is ' + chalk.magenta('LowerCase.');
                    }
                    else if (global.CSSHINT_HEXCOLOR_CASE_FLAG === 1) {
                        str += ', Current project case is ' + chalk.magenta('UpperCase.');
                    }
                    source = decl.source;
                    line = source.start.line;
                    lineContent = util.getLineContent(line, source.input.css);
