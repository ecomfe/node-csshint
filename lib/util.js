 ...
 *                      是一个单独的值例如 1px。`1px solid black` 这样的值是不行的
 *
 * @return {Object} 信息对象
 */
...
exports.getPropertyValue = function (text) {
    ...
    // hexcolor
        ...
        temp = RegExp.$1;
        if (temp.length === 3) {
            ret.red = parseInt(temp.charAt(0) + temp.charAt(0), 16);
            ret.green = parseInt(temp.charAt(1) + temp.charAt(1), 16);
            ret.blue = parseInt(temp.charAt(2) + temp.charAt(2), 16);
        }
        else {
            ret.red = parseInt(temp.substring(0, 2), 16);
            ret.green = parseInt(temp.substring(2, 4), 16);
            ret.blue = parseInt(temp.substring(4, 6), 16);
        }
    }
    ...
    // rgb() color with percentages
        ...
        ret.type = 'color';
        ret.red = +RegExp.$1 * 255 / 100;
        ret.green = +RegExp.$2 * 255 / 100;
        ret.blue = +RegExp.$3 * 255 / 100;
    }
    ...
    // hsl()
        ...
        ret.type = 'color';
        ret.hue = +RegExp.$1;
        ret.saturation = +RegExp.$2 / 100;
        ret.lightness = +RegExp.$3 / 100;
    }
