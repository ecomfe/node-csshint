    ...
    Object.getOwnPropertyNames(
        ...
        function (prop) {
            ...
            if (fs.existsSync(ruleFilePath)) {
                plugins.push(require(path.join(ruleDir, prop))({aaa: 1}));
            }
        }
    );

    // var processor = postcss(plugins);
    // // console.warn(processor.process(fileContent));
    // // console.warn();

    // processor.process(fileContent, {aa: 1}).then(function (result) {
    //     console.log(result.css);
    // });

    // postcss(plugins).process(fileContent).then(function (result1) {
    //     console.log(result1.css, 999);
    // });
    // var result2 = postcss.parse(fileContent).toResult();
    // console.warn(result2);

    var deferred = Q.defer();

    // console.log(postcss(plugins).process(fileContent).css)

    // try {
        // postcss.parse(fileContent);
        postcss(plugins).process(fileContent).then(function (result) {
            result.warnings().forEach(function (message) {
                var source = message.node.source;
                // console.warn(message.node.source, 'ss', filePath);
                // invalidList.push(message.toString());
                invalidList.push({
                    path: filePath,
                    messages: [
                        {
