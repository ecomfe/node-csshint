...
exports.checkString = function (fileContent, filePath, rcConfig) {
    ...
    postcss().use(require(path.join(ruleDir, 'inline-comment'))()).process(fileContent).then(function (result) {
        result.warnings().forEach(function (message) {
            // console.warn(message.node.source, 'ss');
...
// var ENABLE_INLINE_PATTERN = /\/\*+\s*\bcsshint\-enable\b\s*(.*)\s*\*\//gmi;

// var TEST_PATTERN = /\/\*+\s*\bcsshint\-disable\b\s*.*\s*\*\/([\s\S]*)(\/\*+\s*\bcsshint\-enable\b\s*(.*)\s*\*\/)?/gmi;
...
function analyzeInlineRule(fileContent, rcConfig) {
    // TEST_PATTERN.test(fileContent);
    // console.warn(RegExp.$1);
    var ret = {};
    var match = null;
